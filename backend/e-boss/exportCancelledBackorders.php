...
header('Content-Disposition: attachment; filename="cancelled_backorders_export_' . date('Y-m-d_H-i-s') . '.xlsx"');
...
// Set title
$sheet->setTitle('Cancelled Backorders Export');
...
$sheet->getStyle($headerRange)->getFill()
    ...
    ->getStartColor()->setRGB('6C757D'); // Gray color for cancelled items
...
while ($data = mysqli_fetch_assoc($result)) {
    // Calculate aging for cancelled items (static calculation)
    ...
    $sheet->setCellValue('W' . $row, !empty($data['cancel_reason']) ? $data['cancel_reason'] : '—');
