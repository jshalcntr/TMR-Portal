...
header('Content-Disposition: attachment; filename="deleted_backorders_export_' . date('Y-m-d_H-i-s') . '.xlsx"');
...
// Set title
$sheet->setTitle('Deleted Backorders Export');
...
$sheet->getStyle($headerRange)->getFill()
    ...
    ->getStartColor()->setRGB('DC3545'); // Red color for deleted items
...
while ($data = mysqli_fetch_assoc($result)) {
    ...
    $sheet->setCellValue('W' . $row, !empty($data['delete_reason']) ? $data['delete_reason'] : '—');
