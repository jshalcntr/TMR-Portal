...
header('Content-Disposition: attachment; filename="backorders_export_' . date('Y-m-d_H-i-s') . '.xlsx"');
...
$aging_filter = isset($_GET['aging']) ? $_GET['aging'] : '';

// Build query with filters
$query = "SELECT *, (qty * bo_price) AS total 
          FROM backorders_tbl WHERE order_status != 'Cancelled' AND order_status != 'Delivered'";
...
// Set title
$sheet->setTitle('Backorders Export');
...
while ($data = mysqli_fetch_assoc($result)) {
    // Calculate aging and format dates
